...
def _get_config_path():
    ...
    name = ("filename_character_renamer_z_windows.ini"
            if platform.system() == "Windows"
            else "filename_character_renamer_z_linux.ini")
...
STRINGS = {
    "en": {
        # window
        "window_title":        "Filename Character Renamer Z",
        # warning banner
        ...
        # title
        "app_title":           "Remove or Swap Custom Characters in Filenames",
        # dolphin row
        ...
        # substring row
        "substr_label":        "Character(s) to remove/swap:",
        "substr_hint":         "<- Enter the character(s) (example: []abc123 )",
        # swap row
        "swap_cb":             "Swap instead of remove (replace with the text below):",
        "swap_hint":           "(leave empty to just remove the character(s) above)",
        # path bar row
        "path_label":          "Path:",
        ...
        "note_swap":           "Swapping characters: '{s}' -> '{r}'",
    ...
    "pt": {
        ...
        # title
        "app_title":           "Remover ou Trocar Caracteres Personalizados de Nomes de Arquivo",
        # dolphin row
        ...
        # substring row
        "substr_label":        "Caractere(s) a remover/trocar:",
        "substr_hint":         "<- Digite caractere(s) (exemplo: []abc123 )",
        # swap row
        "swap_cb":             "Trocar ao inves de remover (substituir pelo texto abaixo):",
        "swap_hint":           "(deixe vazio para apenas remover o(s) caractere(s) acima)",
        # path bar row
        "path_label":          "Caminho:",
        ...
        "note_swap":           "Trocando caracteres: '{s}' -> '{r}'",
...
class CustomCharacterRemover:
    def __init__(self, root):
        ...
        self._cfg_swap_mode       = False
        self._cfg_swap_value      = ""
        ...
        self.swap_mode.set(self._cfg_swap_mode)
        self.swap_entry.delete(0, tk.END)
        self.swap_entry.insert(0, self._cfg_swap_value)
        if self.last_folder:
    ...
    def _load_config(self):
        ...
        self._cfg_swap_mode       = cfg.getboolean(s, "swap_mode",        fallback=False)
        self._cfg_swap_value      = cfg.get       (s, "swap_value",       fallback="")
    ...
    def _save_config(self):
        ...
        cfg["settings"] = {
            ...
            "swap_mode":       str(self.swap_mode.get()),
            "swap_value":      self.swap_entry.get(),
        }
        try:
    ...
    def _build_ui(self):
        ...
        # Substring input
        self.input_frame = tk.Frame(root, pady=6, padx=10,
                                    relief=tk.GROOVE, bd=1)
        self.input_frame.pack(fill=tk.X, padx=20, pady=(0, 4))
        ...
        # Swap toggle (replace the matched character(s) instead of removing them)
        self.swap_frame = tk.Frame(root, bg="#F5F0FF", pady=6, padx=10,
                                   relief=tk.GROOVE, bd=1)
        self.swap_frame.pack(fill=tk.X, padx=20, pady=(0, 4))
        self.swap_mode = tk.BooleanVar(value=False)
        self.swap_cb_widget = tk.Checkbutton(
            self.swap_frame, text="",
            variable=self.swap_mode,
            font=("Arial", 10, "bold"), fg="#6A3FA0", bg="#F5F0FF",
            wraplength=880,
            command=self._on_swap_toggle,
        )
        self.swap_cb_widget.pack(side=tk.LEFT)
        self.swap_entry = tk.Entry(self.swap_frame, font=("Arial", 11),
                                   width=15, state=tk.DISABLED,
                                   disabledbackground="#D8D8D8")
        self.swap_entry.pack(side=tk.LEFT, padx=(6, 2))
        self.swap_entry.bind("<FocusOut>", lambda e: (self._rescan(), self._save_config()))
        self.swap_entry.bind("<Return>",   lambda e: (self._rescan(), self._save_config()))
        self.lbl_swap_hint = tk.Label(self.swap_frame, text="",
                 font=("Arial", 9), fg="gray", bg="#F5F0FF")
        self.lbl_swap_hint.pack(side=tk.LEFT, padx=4)
        ...
        self.btn_rescan = tk.Button(self.btn_frame, text="",
                  command=self.scan_files,
                  bg="#E0A800", fg="white", padx=20, pady=8,
                  font=("Arial", 10, "bold"))
        self.btn_rescan.pack(side=tk.LEFT, padx=5)
    ...
    def _apply_lang(self):
        ...
        self.swap_cb_widget.config(text=t("swap_cb"))
        self.lbl_swap_hint.config(text=t("swap_hint"))
        ...
        self.btn_rescan.config(text=t("btn_rescan"))
    ...
    def _apply_theme(self):
        ...
        # Swap frame
        self.swap_frame.config(bg=p["swap_bg"])
        self.swap_cb_widget.config(
            bg=p["swap_bg"], fg=p["swap_fg"],
            activebackground=p["swap_bg"], activeforeground=p["swap_fg"],
            selectcolor=p["cb_select"],
        )
        self.lbl_swap_hint.config(bg=p["swap_bg"], fg=p["swap_hint_fg"])
        if str(self.swap_entry.cget("state")) == "normal":
            self.swap_entry.config(bg=p["entry_bg"], fg=p["entry_fg"],
                                   insertbackground=p["entry_ins"])
        else:
            self.swap_entry.config(disabledbackground=p["entry_dis_bg"],
                                   disabledforeground=p["entry_dis_fg"])

        # Input frame
        self.input_frame.config(bg=p["input_bg"])
        self.lbl_substr.config(bg=p["input_bg"], fg=p["substr_box_fg"])
        ...
        self.btn_rescan.config(bg=p["btn_rescan_bg"])
    ...
    def _update_mode(self):
        ...
        if either_on:
            ...
            self.swap_cb_widget.config(state=tk.DISABLED)
            self.swap_entry.config(state=tk.DISABLED,
                                   disabledbackground=p["entry_dis_bg"],
                                   disabledforeground=p["entry_dis_fg"])
        else:
            ...
            self.swap_cb_widget.config(state=tk.NORMAL)
            if self.swap_mode.get():
                self.swap_entry.config(state=tk.NORMAL,
                                       bg=p["entry_bg"], fg=p["entry_fg"],
                                       insertbackground=p["entry_ins"])
            else:
                self.swap_entry.config(state=tk.DISABLED,
    ...
    def _pick_folder(self, initial=""):
        ...
        if platform.system() == "Linux" and shutil.which("kdialog"):
            try:
                ...
                # user did there (picked a folder, or cancelled/closed it)
    ...
    def _build_entry(self, full_path, old_name):
        ...
        # Swap mode: replace matched character(s) with this text instead of
        # deleting them. Empty (default / swap off) reproduces the original
        # remove behaviour exactly.
        replacement = self.swap_entry.get() if self.swap_mode.get() else ""

        after_dolphin = fix_dolphin_duplicate(old_name) if self.fix_dolphin.get() else old_name

        if substring:
            if self.case_sensitive.get():
                after_sub = after_dolphin.replace(substring, replacement)
            else:
                pat = re.compile(re.escape(substring), re.IGNORECASE)
                # Use a lambda (not a raw string) so any backslashes in the
                # replacement text are inserted literally, never treated as
                # regex backreferences.
                after_sub = pat.sub(lambda m: replacement, after_dolphin)
        else:
    ...
    def rename_files(self):
        ...
        if substring:
            if self.swap_mode.get():
                notes.append(self._t("note_swap").format(
                    s=substring, r=self.swap_entry.get()))
            else:
